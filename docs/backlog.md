...
| P1 | Sync validation does not flag a missing cross-submit dependency, and the cause is unknown. Investigated 2026-09-06, superseding this row's original claim that sync validation was off — it is not, `validate_sync` is hardcoded `VK_TRUE`. **Both layers demonstrably work**: a deliberately wrong `oldLayout` fails the case through `ValidationGuard`, and two unbarriered copies to one buffer produce `vkCmdCopyImageToBuffer(): WRITE_AFTER_WRITE hazard detected`. **Yet the original symptom reproduces**: dropping the wait semaphore from `PresentTargetTests`' offscreen read leaves all 18 gpu tests passing, and so does additionally weakening the readback barrier's source scope to `Copy`/`CopySrc`. Forcing syncval through `VK_LAYER_ENABLES` or a layer-settings file with `syncval_submit_time_validation` changes nothing. **Two explanations survive, with opposite consequences.** Either there is no hazard — both submits go to the graphics queue, and a pipeline barrier's first synchronization scope covers commands earlier in submission order, so the barrier supplies the dependency and the semaphore is belt-and-braces, making this row a misdiagnosis to close. Or syncval does not track this hazard class here, and the coverage gap is real. Settling it needs the spec's submission-order and pipeline-barrier scope text, which was not reachable when this was written — registry.khronos.org 403s and docs.vulkan.org truncates the chapter | `tests/gpu/`, `tests/support/GpuReadback.h` | S to settle | |
| P1 | A test proving the gpu suite can see a **cross-submit** hazard — a deliberately unsynchronised read-after-write across two submissions that is required to fail. The suite currently proves only that it sees hazards *inside* one command buffer (verified 2026-09-06). Until it exists, "sync validation was clean" is not evidence about cross-submit ordering, which is exactly what Stage 7.5 step 2 changes when it moves every submit in the engine behind `IDevice` | `tests/gpu/` | S–M | the row above — what the test may assert depends on which explanation holds |
