...
| P2 | The GPU tests may assert a synchronization dependency they cannot detect: dropping the wait semaphore from an offscreen read was found to still pass. **The stated cause is wrong** — this row says "with sync validation off", but `validate_sync` is hardcoded `VK_TRUE` and the GPU tests enable validation unconditionally, so it is on. Re-run the experiment and correct this row: either it predates that line, or it still passes *with* sync validation on, which is worse than described | `tests/gpu/`, `rhi/DeviceDesc.h` | S to diagnose | **do this before Stage 7.5 step 2**, which rests on the answer |
