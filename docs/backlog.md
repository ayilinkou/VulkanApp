...
| P2 | A neutral "can this format be used for this" query on `IDevice`. Depth format selection is the last piece of Vulkan the renderer writes: it asks `VkFormatProperties` which depth formats the hardware supports, through the one non-permanent entry left in the RHI's transitional allowlist. Every other escape-hatch use is ImGui's and stays by design | `rhi/IDevice.h`, `Engine.cpp` | S–M | |
